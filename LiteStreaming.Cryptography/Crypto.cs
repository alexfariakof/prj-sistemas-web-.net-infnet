...
public class Crypto : ICrypto
{
    ...
    {
        var key = options?.Value?.Key ?? throw new ArgumentNullException("Chave Auth não definida.");
        var keyByte = Encoding.UTF8.GetBytes(key);
        this._key = keyByte;
        var authSalt = options?.Value?.AuthSalt ?? throw new ArgumentNullException("Chave Auth Salt não definida.");
        this._authSalt = Encoding.UTF8.GetBytes(authSalt); ;
    }
